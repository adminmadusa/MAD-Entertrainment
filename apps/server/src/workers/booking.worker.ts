...
export async function processBookingConfirm(bookingId: string): Promise<void> {
  ...
  if (existingTicketsCount > 0) {
    logger.warn(
      { bookingId },
      "Idempotency guard triggered: tickets already exist for booking. Skipping.",
    );
    return;
  }
  ...
  if (!event) {
    throw new Error(
      `Event ${booking.eventId} not found for booking ${bookingId}`,
    );
  }
  ...
  logger.info(
    { bookingId },
    "Async ticket generation complete. Enqueued PDF generation task.",
  );
}
...
export function startBookingWorker(): void {
  ...
  localFallbackEmitter.on(QUEUE_NAME, async (job) => {
    logger.info(
      { jobId: job.id },
      "Processing booking confirm job via local EventEmitter fallback",
    );
    try {
      ...
      logger.error(
        { err, jobId: job.id },
        "Local booking confirm job fallback execution failed",
      );
    }
  });
  ...
  if (!isRedisConnected()) {
    logger.warn(
      "Redis offline. Operating booking worker in in-memory degraded fallback mode.",
    );
    return;
  }
  ...
  try {
    ...
    worker = new Worker(
      ...
      async (job: Job) => {
        logger.info(
          { jobId: job.id },
          "Processing booking confirm job via BullMQ",
        );
    ...
    worker.on("failed", async (job, err) => {
      logger.error(
        { err, jobId: job?.id },
        "Booking confirm job failed in BullMQ",
      );
      if (job && job.attemptsMade >= (job.opts.attempts || 3)) {
        ...
        try {
          ...
          logger.warn(
            { jobId: job.id },
            "Job moved to Dead-Letter Queue database collection.",
          );
        } catch (dlqErr) {
          logger.error(
    ...
    logger.error(
      { err },
      "Failed to start BullMQ Booking Worker. Degraded mode active.",
    );
  }
}
