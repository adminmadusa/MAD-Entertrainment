...
export const createTier = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    res.status(201).json({ success: true, data: { tier }, message: 'Tier created successfully' });
  } catch (error) {
    next(error);
  }
};
...
export const getTiers = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    res.status(200).json({ success: true, data: result, message: 'Tiers fetched successfully' });
  } catch (error) {
    next(error);
  }
};
