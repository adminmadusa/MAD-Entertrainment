...
async function verifySeatsAvailable(
  eventId: string,
  seatIds: string[],
): Promise<boolean> {
  try {
    ...
    logger.error(
      { err, eventId, seatIds },
      "Failed to verify seats in database",
    );
    return false;
  }
}
...
async function filterSeatsStillReservedInDatabase(
  eventId: string,
  seatIds: string[],
): Promise<string[]> {
  const layout = await SeatLayout.findOne({ eventId })
  ...
  const unavailable = new Set(
    layout.seats
      .filter(
        (seat) =>
          seatIds.includes(seat.seatId) && seat.status !== SeatStatus.AVAILABLE,
      )
...
function withSocketContext(
  ...
  return (...args: any[]) => {
    const parentId =
      socket.data.correlationId || `socket:conn:${crypto.randomUUID()}`;
...
export function registerSocketHandlers(socket: Socket): void {
  ...
  socket.on(
    "event:join",
    withSocketContext(socket, "event:join", async (payload: unknown) => {
      const parseResult = socketEventJoinSchema.safeParse(payload);
      if (!parseResult.success) {
        socket.emit("event:join:status", {
          success: false,
    ...
    withSocketContext(socket, "seat:lock", async (payload: unknown) => {
      ...
      if (socket.data.sessionId && socket.data.sessionId !== sessionId) {
        ...
        logger.warn(
          {
            socketId: socket.id,
            expectedSessionId: socket.data.sessionId,
      ...
      if (isRedisConnected()) {
        ...
        logger.warn(
          "Redis disconnected. Falling back to MongoDB for seat validation (degraded mode).",
        );
      ...
      if (success) {
        ...
        auditLog({
          ...
          metadata: {
            ...
            seatIds,
            sessionId,
            reason: "Already locked or booked",
          },
  ...
  socket.on(
    ...
    withSocketContext(socket, "seat:unlock", async (payload: unknown) => {
      const parseResult = socketSeatActionSchema.safeParse(payload);
      if (!parseResult.success) {
        socket.emit("seat:unlock:status", {
          success: false,
          message: "Invalid payload format",
        });
        socket.emit("error", {
      ...
      if (releasedSeatIds.length > 0) {
        socket
          .to(`event:${eventId}`)
          .emit("seat:unlocked", { seatIds: releasedSeatIds });
      }
      socket.emit("seat:unlock:status", {
        success: true,
  ...
  socket.on(
    "booking:join",
    withSocketContext(socket, "booking:join", async (payload: unknown) => {
      const parseResult = socketBookingJoinSchema.safeParse(payload);
      if (!parseResult.success) {
        socket.emit("booking:join:status", {
          success: false,
          message: "Invalid payload format",
        });
        socket.emit("error", {
      ...
      try {
        ...
        if (!isUserOwner && !isGuestOwner) {
          ...
          logger.warn(
            { socketId: socket.id, bookingId, reqUserId, reqSessionId },
            "Unauthorized booking room join attempt blocked",
          );
          auditLog({
  ...
  socket.on(
    "disconnect",
    withSocketContext(socket, "disconnect", async (reason) => {
      ...
      if (locks && locks.size > 0) {
        ...
        for (const [eventId, seatIds] of grouped.entries()) {
          ...
          const releasedKeys = await releaseLocks(eventId, seatIds, sessionId);
          const releasedSeatIds = releasedKeys
            .map((key) => key.split(":").at(-1))
            .filter(Boolean) as string[];
          const safeToAnnounce = await filterSeatsStillReservedInDatabase(
            eventId,
            releasedSeatIds,
          );
          if (safeToAnnounce.length > 0) {
            socket
              .to(`event:${eventId}`)
              .emit("seat:unlocked", { seatIds: safeToAnnounce });
          }
          ...
          logger.info(
            {
              ...
              eventId,
              seatIds: releasedSeatIds,
              announcedSeatIds: safeToAnnounce,
            },
      ...
      auditLog({
        ...
        metadata: {
          socketId: socket.id,
          reason,
          releasedSeatLocks: releasedList,
        },
...
export function registerAdminSocketHandlers(socket: Socket): void {
    ...
    withAdminSocketContext(
      ...
      ({ room }: { room: string }) => {
        if (!socket.data.admin || !socket.data.admin.role) {
          logger.warn(
            { socketId: socket.id },
            "Unauthorized admin:join attempt (missing credentials)",
          );
          socket.emit("admin:join:status", {
            success: false,
            room,
        ...
        if (!allowedRooms.includes(room)) {
          logger.warn(
            { socketId: socket.id, room, admin: socket.data.admin },
            "Forbidden admin:join attempt for unauthorized room",
          );
          socket.emit("admin:join:status", {
            success: false,
            room,
  ...
  socket.on(
    "disconnect",
    withAdminSocketContext(socket, "disconnect", () => {
      auditLog({
