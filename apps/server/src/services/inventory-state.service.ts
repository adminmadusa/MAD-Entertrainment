...
const INVENTORY_TRANSITIONS: Record<InventoryState, InventoryState[]> = {
  ...
  [InventoryState.RESERVED]: [
    InventoryState.PENDING_PAYMENT,
    InventoryState.EXPIRED,
  ],
  [InventoryState.PENDING_PAYMENT]: [
    InventoryState.BOOKED,
...
export function assertInventoryTransition(
  ...
  if (!INVENTORY_TRANSITIONS[from]?.includes(to)) {
    logger.warn(
      { from, to, ...context },
      "Invalid inventory transition rejected",
    );
...
export function assertReservationTransition(
  ...
  if (!RESERVATION_TRANSITIONS[from]?.includes(to)) {
    logger.warn(
      { from, to, ...context },
      "Invalid reservation transition rejected",
    );
...
export function reservationToInventoryState(
