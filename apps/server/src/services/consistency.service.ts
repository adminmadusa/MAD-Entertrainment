...
async function countEventInventoryMismatches(): Promise<number> {
  const events = await Event.find({})
    .select("_id soldCount reservedCount")
    .lean();
  let mismatches = 0;
...
async function repairEventInventoryMismatches(): Promise<number> {
  const events = await Event.find({}).select(
    "_id soldCount reservedCount ticketTiers eventVersion",
  );
  let repairedCount = 0;
  ...
  for (const event of events) {
    ...
    if (
      ...
      const confirmedBookingsDocs = await Booking.find({
        eventId: event._id,
      ...
      for (const bookingDoc of confirmedBookingsDocs) {
        for (const t of bookingDoc.tickets) {
          tierSoldCounts.set(
            t.tier,
            (tierSoldCounts.get(t.tier) ?? 0) + t.quantity,
          );
        }
      }
  ...
  static async runRepairCycle(): Promise<ConsistencyReport> {
    ...
    return runWithContext({ correlationId }, async () => {
      ...
      if (hasRepairs) {
        logger.warn(
          { report },
          "Consistency repair cycle completed with repairs",
        );
        emitToAdmin("inventory", "consistency:repaired", report);
        for (const [eventId, reservations] of ReservationService.groupByEvent(
          expiredReservations,
        ).entries()) {
