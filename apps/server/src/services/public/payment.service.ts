...
export class PaymentService {
  ...
  private static async handleRazorpayIntent(
    ...
    if (env.MOCK_PAYMENTS) {
      const mockOrderId =
        "order_mock_" + Math.random().toString(36).substring(2, 10);
    ...
    if (!isRazorpayEnabled()) {
      throw AppError.badRequest(
        "Razorpay is not enabled / credentials missing",
      );
    }
  ...
  private static async handleStripeIntent(
    ...
    try {
      ...
      const statusCode = err.statusCode || 400;
      const description =
        err.message || "Stripe payment intent creation failed";
      throw new AppError(
  ...
  static async verifyPayment(bookingId: string, gatewayPayload: any) {
    ...
    if (payment.gateway === "razorpay") {
      ...
      if (!razorpay_order_id || !razorpay_payment_id || !razorpay_signature) {
        throw AppError.badRequest(
          "Missing Razorpay credentials in payment payload",
        );
      }
      ...
      const isMock =
        env.MOCK_PAYMENTS && paymentIntentId.startsWith("pi_mock_");
      ...
      if (isMock) {
        ...
        if (intent.status !== "succeeded") {
          ...
          throw AppError.badRequest(
            `Stripe payment verification failed. Status is "${intent.status}"`,
          );
        }
        ...
        if (intentBookingId !== booking._id.toString()) {
          ...
          throw AppError.badRequest(
            "Stripe payment intent does not belong to this booking",
          );
        }
        ...
        if (
          ...
          throw AppError.badRequest(
            "Stripe payment intent booking reference mismatch",
          );
        }
  ...
  private static async failPaymentAndReleaseInventory(
    ...
    logger.info(
      ...
      "Payment failed and reserved inventory released",
    );
  }

  private static async confirmBooking(
    booking: IBooking,
    _payment: IPayment,
    ...
    const confirmedReservations = await ReservationService.transitionForBooking(
      booking._id,
    ...
    if (event) {
      ...
      for (const bookedTicket of booking.tickets) {
        ...
        if (tierIndex !== -1) {
          incUpdate[`ticketTiers.${tierIndex}.soldCount`] =
      ...
      if (
        updatedEvent &&
        updatedEvent.soldCount >= updatedEvent.totalCapacity &&
    ...
    if (getEnv().ENABLE_ASYNC_CHECKOUT) {
      ...
      logger.info(
        { bookingId: booking._id },
        "Asynchronous checkout enabled. Handed off confirmation tasks to background queue.",
      );
      return booking;
    }
    ...
    try {
      ...
      logger.info(
        { bookingId: booking._id },
        "Notification log created for confirmed booking and email dispatched",
      );
    } catch (err) {
      logger.error(
        { err },
        "Failed to record notification confirmation log or send email",
      );
    }
    return booking;
  }
