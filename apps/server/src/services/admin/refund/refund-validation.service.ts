  ...
  static assertProductionRefundIntegrity(
    ...
    for (const id of identifiers) {
      ...
      if (mockPatterns.some((pattern) => id.includes(pattern))) {
        ...
        const errorMsg = 'MOCK_PAYMENT_IDENTIFIER_DETECTED: Mock identifier submitted in production.';
        const localMetadata = { ...metadata, paymentId: sanitizedId };
        logger.error(localMetadata, 'Mock identifier detected in production environment');
        auditLog({
