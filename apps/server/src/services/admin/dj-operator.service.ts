...
export const getDJOperators = async (
  page: number = 1,
  limit: number = 10,
): Promise<{ djs: IDJOperator[]; total: number; pages: number }> => {
  const skip = (page - 1) * limit;
  const total = await DJOperator.countDocuments({ isDeleted: { $ne: true } });
  const djs = await DJOperator.find({ isDeleted: { $ne: true } })
    .sort({ createdAt: -1 })
    .skip(skip)
    .limit(limit);
  return {
...
export const deleteDJOperator = async (
  ...
  const deleted = await DJOperator.findByIdAndUpdate(
    id,
