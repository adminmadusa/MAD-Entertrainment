...
describe('Cache Service', () => {
  ...
  describe('set', () => {
    ...
    it('should set value without TTL expiration if set to 0', async () => {
      ...
      expect(mockRedis.set).toHaveBeenCalledWith('mad:cache:user:role', JSON.stringify(value));
    });
  });
  ...
  describe('delPattern', () => {
    it('should scan and delete all matching keys', async () => {
      vi.mocked(isRedisConnected).mockReturnValue(true);
      mockRedis.scan
        .mockResolvedValueOnce(['next_cursor', ['mad:cache:events:1', 'mad:cache:events:2']])
        .mockResolvedValueOnce(['0', ['mad:cache:events:3']]);

      await CacheService.delPattern('events:*');

      expect(mockRedis.scan).toHaveBeenCalledTimes(2);
      expect(mockRedis.del).toHaveBeenCalledWith('mad:cache:events:1', 'mad:cache:events:2');
