...
} from "./workers/consistency.worker";
...
async function bootstrap(): Promise<void> {
  ...
  try {
    ...
    logger.warn(
      { err },
      "Redis connection failed during bootstrap. Starting in degraded mode.",
    );
  }
  ...
  httpServer.listen(PORT, () => {
    logger.info(
      `🚀 MAD Entertrainment API running on http://localhost:${PORT}`,
    );
