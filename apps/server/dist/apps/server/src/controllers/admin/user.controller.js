...
async function listAdmins(req, res) {
    ...
    const { admins, total } = await user_service_1.AdminUserService.listAdmins(page, limit);
    (0, response_1.sendPaginated)(res, admins, (0, response_1.buildPaginationMeta)(total, page, limit));
}
async function createAdminUser(req, res) {
    const body = req.body;
    const admin = await user_service_1.AdminUserService.createAdminUser(body);
...
async function toggleAdminActive(req, res) {
    ...
    const isActive = await user_service_1.AdminUserService.toggleAdminActive(req.params.id, currentAdminId);
