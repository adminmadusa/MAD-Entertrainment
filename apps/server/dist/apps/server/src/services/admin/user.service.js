...
class AdminUserService {
    static async listAdmins(page, limit) {
        ...
        const [admins, total] = await Promise.all([
            admin_schema_1.Admin.find()
                .sort({ createdAt: -1 })
                .skip(skip)
                .limit(limit)
                .select('-passwordHash')
                .lean(),
            admin_schema_1.Admin.countDocuments(),
        ]);
        return { admins, total };
    }
    static async createAdminUser(data) {
        ...
        const existing = await admin_schema_1.Admin.findOne({ email: email.toLowerCase() });
        if (existing) {
        ...
        const admin = await admin_schema_1.Admin.create({
            name,
    ...
    static async toggleAdminActive(id, currentAdminId) {
        ...
        const admin = await admin_schema_1.Admin.findById(id);
        if (!admin) {
