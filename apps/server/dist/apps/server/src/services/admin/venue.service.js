...
class VenueService {
    ...
    static async getVenueById(id) {
        const venue = await venue_schema_1.Venue.findById(id);
        if (!venue) {
    ...
    static async createVenue(data) {
        ...
        const existing = await venue_schema_1.Venue.findOne({ slug });
        if (existing) {
            throw error_middleware_1.AppError.conflict(`Slug "${slug}" is already in use.`);
        }
        return await venue_schema_1.Venue.create({ ...data, slug });
    }
    /**
    ...
    static async updateVenue(id, data) {
        if (data.slug) {
            const existing = await venue_schema_1.Venue.findOne({ slug: data.slug, _id: { $ne: id } });
            if (existing) {
                throw error_middleware_1.AppError.conflict(`Slug "${data.slug}" is already in use.`);
            }
        }
        const venue = await venue_schema_1.Venue.findByIdAndUpdate(id, data, {
            new: true,
            runValidators: true,
        });
        if (!venue) {
    ...
    static async deleteVenue(id) {
        const venue = await venue_schema_1.Venue.findByIdAndDelete(id);
        if (!venue) {
            throw error_middleware_1.AppError.notFound('Venue');
        }
    }
