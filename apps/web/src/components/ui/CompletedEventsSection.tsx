...
export const CompletedEventsSection = memo(function CompletedEventsSection({
  ...
  return (
    <section className="py-16 overflow-hidden bg-background/20" aria-label="Past events recap" role="region">
              ...
              <Reveal key={event._id} delay={index * 100}>
                ...
                >
                  <Link
                    ...
                    aria-label={`View recap for completed event ${event.title}`}
                  >
                    ...
                    <div className="aspect-[4/3] w-full overflow-hidden relative bg-white/5 flex-shrink-0">
                      {event.bannerImage?.url ? (
                        ...
                        <div className="w-full h-full flex items-center justify-center text-accent-pink text-5xl" aria-hidden="true">
                          🎧
                        </div>
                      )}

                      {/* Black Overlay with Ended Badge */}
                      <div className="absolute inset-0 bg-black/40 group-hover:bg-black/20 transition-colors" />

                      {/* Ended indicator */}
                      <span className="absolute top-3 left-3 px-2.5 py-1 text-[10px] font-black uppercase tracking-wider bg-black/80 backdrop-blur-md text-text-muted rounded-full border border-white/10">
                        Ended
                      </span>
