...
export async function invalidatePublicBookingFlow(
  ...
  await Promise.all([
    ...
    queryClient.invalidateQueries({
      queryKey: QUERY_KEYS.public.bookings.checkout(
        details.bookingRef ?? details.bookingId,
      ),
    }),
    queryClient.invalidateQueries({
      queryKey: QUERY_KEYS.public.bookings.detail(
        details.bookingRef ?? details.bookingId,
      ),
    }),
    queryClient.invalidateQueries({
    ...
    queryClient.invalidateQueries({
      queryKey: QUERY_KEYS.public.events.detail(details.eventSlug),
    }),
    queryClient.invalidateQueries({
      queryKey: QUERY_KEYS.public.events.seats(details.eventId),
    }),
  ]);
}
