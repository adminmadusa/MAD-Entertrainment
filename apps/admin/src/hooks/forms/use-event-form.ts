...
} from "@/lib/mappers/event-form.mapper";
...
interface UseEventFormOptions {
  ...
  onSubmitPayload: (
    payload: ReturnType<typeof mapFormValuesToPayload>,
  ) => Promise<void> | void;
}
...
export function useEventForm({
  ...
  const [values, setValues] = useState<EventFormValues>(
    mode === "edit" && initialEvent
      ? mapEventToFormValues(initialEvent)
      : defaultValues,
  );
  ...
  const addTier = () =>
    setValues((prev) => ({ ...prev, tiers: [...prev.tiers, defaultTier()] }));
  ...
  const updateTier = (
    index: number,
    field: keyof TicketTierFormValues,
    value: unknown,
  ) =>
    setValues((prev) => ({
      ...prev,
