...
interface UseTicketProfileFormOptions {
  ...
  onSubmitPayload: (
    payload: ReturnType<typeof mapTicketProfileFormToPayload>,
  ) => Promise<void> | void;
}

export function useTicketProfileForm({
  mode,
  initialProfile,
  onSubmitPayload,
}: UseTicketProfileFormOptions) {
  const [values, setValues] = useState<TicketProfileFormValues>(
    mode === "edit" && initialProfile
      ? mapTicketProfileToFormValues(initialProfile)
      : getDefaultTicketProfileFormValues(),
  );
  const [error, setError] = useState("");

  const setField = <K extends keyof TicketProfileFormValues>(
    field: K,
  ...
  const addGroup = () =>
    setValues((prev) => ({
      ...prev,
      groups: appendAt(prev.groups, defaultGroup()),
    }));
  const removeGroup = (groupIndex: number) =>
    setValues((prev) => ({
      ...prev,
      groups: removeAt(prev.groups, groupIndex),
    }));
  ...
  const addTicket = (groupIndex: number) => {
    setValues((prev) => ({
      ...
      groups: updateAt(prev.groups, groupIndex, (group) => ({
        ...group,
        tickets: appendAt(group.tickets, defaultTicket()),
      })),
    }));
  };
  ...
  const removeTicket = (groupIndex: number, ticketIndex: number) => {
    setValues((prev) => ({
      ...
      groups: updateAt(prev.groups, groupIndex, (group) => ({
        ...group,
        tickets: removeAt(group.tickets, ticketIndex),
      })),
    }));
  };
      ...
      groups: updateAt(prev.groups, groupIndex, (group) => ({
        ...
        tickets: updateAt(group.tickets, ticketIndex, (ticket) => {
          if (field === "price" && value === 0)
            return { ...ticket, price: 0, isFree: true };
