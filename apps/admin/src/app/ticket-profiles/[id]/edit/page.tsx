...
export default function EditTicketProfilePage() {
  ...
  const { data: profile, isLoading } = useQuery({ queryKey: ['admin-ticket-profile', id], queryFn: () => adminGetTicketProfile(id) });

  useEffect(() => {
    if (profile) {
      setName(profile.name || '');
      setDescription(profile.description || '');
      setGroups((profile.groups || []).map((g) => ({
        name: g.name, slug: g.slug, description: g.description || '',
        tickets: (g.tickets || []).map((t) => ({
          tier: t.tier, name: t.name, description: t.description || '',
          price: t.price, isFree: !!t.isFree, totalCapacity: t.totalCapacity,
          minPerBooking: t.minPerBooking || 1, maxPerBooking: t.maxPerBooking || 10, groupSize: t.groupSize || 1,
          discountType: t.offerRules?.discountType || 'none', discountValue: t.offerRules?.discountValue || '',
          minQtyRequired: t.offerRules?.minQtyRequired || 1, buyQty: t.offerRules?.buyQty || '',
          freeTicketQty: t.offerRules?.freeTicketQty || '', isActive: t.isActive !== false,
        })),
      })));
    }
  }, [profile]);
  ...
  const updateMutation = useMutation({
    mutationFn: (payload: ReturnType<typeof buildTicketProfilePayload>) => adminUpdateTicketProfile(id, payload),
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateMutation.mutate(buildTicketProfilePayload(name, description, groups));
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 text-white">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-black text-white">Edit Ticket Profile</h1>
          <p className="text-text-muted text-sm mt-0.5">Modify centralized event ticket template</p>
        </div>
        <button
          onClick={() => router.back()}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <ProfileFormActions submitLabel="Save Changes" isPending={updateMutation.isPending} onCancel={() => router.back()} />
      </form>
    </div>
  );
}
