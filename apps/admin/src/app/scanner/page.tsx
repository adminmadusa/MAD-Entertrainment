...
export default function ScannerPage() {
  ...
  const syncOfflineScans = useCallback(async () => {
    if (isOffline) return;
    const { getOfflineScans, clearOfflineScans } =
      await import("@/lib/offline-scanner.service");
          ...
          >
            ...
            <div className="mt-4 px-6 py-3 bg-emerald-500/10 border border-emerald-500/20 rounded-xl">
              <p className="text-emerald-400 font-bold text-lg">
                ADMIT: {lastScanResult.admits}{" "}
                {lastScanResult.admits > 1 ? "PEOPLE" : "PERSON"}
              </p>
              <p className="text-text-muted text-xs uppercase mt-1 tracking-wider">
