...
export default function AdminAnalyticsPage() {
  ...
  const stats = [
    ...
    {
      label: "Last 30 Days",
      value: summary?.recentBookings?.toLocaleString("en-IN") ?? "—",
      icon: "📅",
    },
    {
      label: "Total Revenue",
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
        {stats.map((stat) => (
          <div
            key={stat.label}
            className="glass rounded-2xl border border-border-subtle p-6"
          >
            <div className="text-3xl mb-3">{stat.icon}</div>
            <p className="text-text-muted text-sm">{stat.label}</p>
            <p
              className={`text-2xl font-black mt-1 ${isLoading ? "text-text-muted animate-pulse" : "text-white"}`}
            >
            ...
            <tbody>
              {summary.topEvents.map((ev) => (
                ...
                >
                  ...
                  <td className="py-3.5 px-6 text-right text-white font-semibold">
