...
export default function DiagnosticsPage() {
  ...
  const hasDrift =
    !!report && Object.values(report.drift).some((value) => value > 0);
  ...
  return (
    <div className="space-y-6">
      ...
      <div
        className={`rounded-2xl border p-4 ${hasDrift ? "border-yellow-500/30 bg-yellow-500/10" : "border-green-500/30 bg-green-500/10"}`}
      >
          ...
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-border-subtle text-text-muted">
                {[
                  ...
                  <th key={heading} className="text-left font-medium py-3 px-4">
                    {heading}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {(reservations ?? []).map((reservation) => (
                ...
                >
                  <td className="py-3 px-4 font-mono text-accent-purple">
                  ...
                  <td className="py-3 px-4 text-text-secondary">
                    {reservation.seatId ?? reservation.section ?? "-"}
                  </td>
                  <td className="py-3 px-4 text-text-secondary">
...
function MetricCard({
  ...
  return (
    <div
      className={`glass rounded-xl border p-4 ${tone === "warn" ? "border-yellow-500/30" : tone === "ok" ? "border-green-500/30" : "border-border-subtle"}`}
    >
