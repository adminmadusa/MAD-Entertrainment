...
export default function AdminRefundsPage() {
  ...
  const processMutation = useMutation({
    mutationFn: () =>
      adminProcessRefund(processTarget!._id, action, adminNotes, gatewayId),
    onSuccess: () => {
  ...
  if (error) {
    return (
      <div className="py-12">
        <ErrorState
          message={(error as Error).message || "Failed to load refunds."}
        />
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-text-muted text-sm mt-0.5">
            {pagination?.total ?? 0} refund requests
          </p>
        </div>
        <select
          value={statusFilter}
            ...
            <thead>
              <tr className="border-b border-border-subtle">
                {[
                  ...
                  <th
                    key={h}
                    className="text-left text-text-muted font-medium py-3.5 px-4"
                  >
                    {h}
                  </th>
                ))}
                ...
                refunds.map((refund) => (
                  ...
                  >
                    <td className="py-3.5 px-4 font-mono text-xs text-accent-purple">
                      {(refund.bookingId as { bookingId?: string })
                        ?.bookingId ?? String(refund.bookingId).slice(-8)}
                    </td>
                    <td className="py-3.5 px-4 text-white font-semibold">
                      ₹{refund.amount.toLocaleString("en-IN")}
                    </td>
                    <td className="py-3.5 px-4 text-text-secondary max-w-40 truncate">
                      {refund.reason ?? "—"}
                    </td>
                    <td className="py-3.5 px-4">
                      <span
                        className={`text-xs px-2.5 py-1 rounded-full border font-medium ${STATUS_COLORS[refund.status] ?? ""}`}
                      >
                        {refund.status}
                      </span>
                    </td>
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4 sm:p-6 overflow-y-auto">
            ...
            >
              <h3 className="text-white font-bold">
                Process Refund — ₹{processTarget.amount.toLocaleString("en-IN")}
              </h3>
