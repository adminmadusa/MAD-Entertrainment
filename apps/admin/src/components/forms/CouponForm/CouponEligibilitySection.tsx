        ...
        <div>
          ...
          {isLoadingEvents ? (
            ...
            <div className="max-h-48 overflow-y-auto border border-border-subtle bg-white/2 rounded-xl p-3 space-y-2 custom-scrollbar">
              {events.map((event) => {
                const isSelected = values.applicableEventIds.includes(event._id);
                return (
                  <div
                    key={event._id}
              ...
              {events.length === 0 && <div className="text-xs text-text-muted text-center py-4">No events found.</div>}
            </div>
          )}
        </div>
      </div>
