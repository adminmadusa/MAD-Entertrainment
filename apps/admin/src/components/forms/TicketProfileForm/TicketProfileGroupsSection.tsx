...
import { TicketProfileFormValues, TicketProfileTicketFormValues } from '@/types/ticket-profile-form';
...
export function TicketProfileGroupsSection({
  ...
  updateGroupField: (groupIndex: number, field: 'name' | 'slug' | 'description', value: string) => void;
  addTicket: (groupIndex: number) => void;
  removeTicket: (groupIndex: number, ticketIndex: number) => void;
  updateTicketField: (groupIndex: number, ticketIndex: number, field: keyof TicketProfileTicketFormValues, value: unknown) => void;
}) {
  return (
