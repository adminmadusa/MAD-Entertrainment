...
export function EventTicketingSection({
  ...
  const activeProfile = profiles.find(
    (profile) => profile._id === values.selectedProfileId,
  );
      ...
      {values.ticketingType === "custom" ? (
                    ...
                    {tiers.length > 0
                      ? tiers.map((item) => (
                          <option
                            key={item._id}
                            value={item.slug}
                            className="bg-background-card capitalize"
                          >
                            {item.name}
                          </option>
                        ))
                      : EVENT_FORM_FALLBACK_TIER_NAMES.map((name) => (
                          <option
                            key={name}
                            value={name}
                            className="bg-background-card capitalize"
        ...
        <div className="space-y-4">
          <FormField label="Select Ticket Profile *">
            <select
              value={values.selectedProfileId}
              onChange={(e) => resetOverridesForProfile(e.target.value)}
              className={inputCls}
            >
          ...
          {activeProfile?.groups?.map((group, gIdx) => (
            ...
            >
              <h4 className="text-accent-purple-light font-bold text-sm">
                {group.name}
              </h4>
              {group.tickets?.map((ticket, tIdx) => {
                ...
                return (
                  <div
                    ...
                    className="p-3 bg-background rounded-lg border border-border-subtle flex flex-wrap items-center gap-3"
                  >
                    ...
                    <input
                      type="number"
                      min={1}
                      value={override.totalCapacity ?? ""}
                      placeholder={`${ticket.totalCapacity}`}
                      onChange={(e) =>
                        setOverride(
                          ticket.tier,
                          "totalCapacity",
                      ...
                      className="w-24 px-2 py-1 rounded bg-background-card border border-border-subtle text-xs"
                    />
                    <label className="text-xs flex items-center gap-1">
                      <input
                        ...
                        onChange={(e) =>
                          setOverride(ticket.tier, "isActive", e.target.checked)
                        }
                      />
                      Visible
                    </label>
