...
export function PopupTargetingSection({
  values,
  onFieldChange,
}: PopupTargetingSectionProps) {
  return (
    <FormSection title="Scope & Targeting">
      <FormField label="Show on pages (comma-separated, blank for all)">
        <input
          id="popup-show-on-pages"
          value={values.showOnPages}
          onChange={(e) => onFieldChange("showOnPages", e.target.value)}
      ...
      <FormField label="Linked Event ID (optional)">
        <input
          id="popup-linked-event-id"
          value={values.linkedEventId}
          onChange={(e) => onFieldChange("linkedEventId", e.target.value)}
