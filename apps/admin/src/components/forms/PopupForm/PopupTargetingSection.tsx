  ...
  return (
    <FormSection title="Scope & Targeting">
      <FormField label="Show on pages (comma-separated, blank for all)">
        <input id="popup-show-on-pages" value={values.showOnPages} onChange={(e) => onFieldChange('showOnPages', e.target.value)} placeholder="e.g. /, /events, /venues" className={POPUP_INPUT_CLASSNAME} />
      </FormField>
      <FormField label="Linked Event ID (optional)">
        <input id="popup-linked-event-id" value={values.linkedEventId} onChange={(e) => onFieldChange('linkedEventId', e.target.value)} placeholder="e.g. 6a11621b76456c3977198702" className={POPUP_INPUT_CLASSNAME} />
      </FormField>
    </FormSection>
  );
}
