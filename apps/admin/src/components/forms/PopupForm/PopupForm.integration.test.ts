...
vi.mock('@/components/CloudinaryUpload', () => ({
  CloudinaryUpload: ({ label }: { label: string }) => React.createElement('div', { 'data-testid': 'cloudinary-upload' }, label),
}));
...
describe('PopupForm integration', () => {
  ...
  it('renders core sections and default visibility state', () => {
    ...
    expect(screen.getByText(/Visibility:/).textContent).toContain('Visibility: active');
  });
  ...
  it('updates visibility state from scheduled to expired based on date inputs', async () => {
    ...
    await waitFor(() => expect(screen.getByText(/Visibility:/).textContent).toContain('Visibility: scheduled'));

    await user.clear(byId('popup-start-date'));
    await user.type(byId('popup-end-date'), '2000-01-01T10:00');
    await waitFor(() => expect(screen.getByText(/Visibility:/).textContent).toContain('Visibility: expired'));
  });
