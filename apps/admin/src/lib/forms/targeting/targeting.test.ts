...
describe("forms/targeting", () => {
  ...
  it("transforms payloads", () => {
    expect(
      toCouponTargetingPayload([" evt1 ", ""], [EventCategory.CONCERT])
        .applicableEventIds,
    ).toEqual(["evt1"]);
    expect(toPopupTargetingPayload("/, /events", " 123 ").linkedEventId).toBe(
      "123",
    );
  });

  it("validates and matches targeting", () => {
    expect(validateTargetingRules({ eventIds: ["e1"], pages: ["/"] })).toBe(
      true,
    );
    expect(matchesEventId(["e1"], "e1")).toBe(true);
    expect(
      matchesCategory([EventCategory.CONCERT], EventCategory.CONCERT),
    ).toBe(true);
