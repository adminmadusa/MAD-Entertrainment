...
export const couponFormSchema = z
  ...
  .superRefine((values, ctx) => {
    ...
    if (values.discountType === 'percentage' && !isPercentageInRange(values.discountValue)) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['discountValue'], message: 'Percentage discount must be between 1 and 100.' });
    }

    if (values.discountType === 'fixed' && !isPositiveNumber(values.discountValue)) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['discountValue'], message: 'Fixed discount amount must be greater than 0.' });
    }

    if (values.maxDiscount !== '' && typeof values.maxDiscount === 'number' && values.maxDiscount < 0) {
      ctx.addIssue({ code: z.ZodIssueCode.custom, path: ['maxDiscount'], message: 'Max discount cannot be negative.' });
    }
