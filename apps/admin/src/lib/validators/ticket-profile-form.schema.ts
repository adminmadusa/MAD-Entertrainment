...
export const ticketProfileGroupSchema = z.object({
  ...
  slug: z.string(),
  description: z.string(),
  tickets: z.array(ticketProfileTicketSchema).min(1, 'Each group must contain at least one ticket'),
});

export const ticketProfileFormSchema = z.object({
  name: z.string().trim().min(1, 'Profile name is required'),
  description: z.string(),
  groups: z.array(ticketProfileGroupSchema).min(1, 'At least one ticket group is required'),
});
