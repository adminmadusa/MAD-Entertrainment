...
describe('popupFormSchema', () => {
  ...
  it('rejects invalid date range', () => {
    expect(popupFormSchema.safeParse({ ...validValues, startDate: '2026-10-11T10:00', endDate: '2026-10-10T10:00' }).success).toBe(false);
  });

  it('rejects invalid ctaUrl protocol', () => {
    expect(popupFormSchema.safeParse({ ...validValues, ctaUrl: 'mad.com/events' }).success).toBe(false);
  });
});
