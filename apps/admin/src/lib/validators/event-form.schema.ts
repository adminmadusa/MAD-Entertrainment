...
export const eventFormSchema = z.object({
  ...
  if (data.ticketingType === 'custom' && data.tiers.length === 0) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'At least one custom tier is required', path: ['tiers'] });
  }
  if (data.ticketingType === 'profile' && !data.selectedProfileId) {
    ctx.addIssue({ code: z.ZodIssueCode.custom, message: 'Ticket profile is required', path: ['selectedProfileId'] });
  }
});
