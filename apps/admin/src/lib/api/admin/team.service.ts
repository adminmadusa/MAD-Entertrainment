...
export async function adminGetAdmins(
  page = 1,
  limit = 15,
): Promise<NormalizedAdminsResponse> {
  try {
    const { data } = await adminApiClient.get<AdminsResponse>(
    ...
    console.error(
      "[Team Service] Failed to fetch team members, returning safe default NormalizedAdminsResponse:",
      error,
    );
    return {
      items: [],
...
export async function adminCreateAdmin(
  ...
  try {
    const { data } = await adminApiClient.post<{ data: Admin }>(
      "/admin/team",
      payload,
    );
    return data.data;
...
export async function adminToggleAdminActive(
  id: string,
): Promise<{ isActive: boolean } | null> {
  try {
    const { data } = await adminApiClient.patch<{
      data: { isActive: boolean };
    }>(`/admin/team/${id}/toggle`);
    return data.data;
  } catch (error) {
    console.error(
      `[Team Service] Failed to toggle active status for team member ${id}:`,
      error,
    );
    throw error;
  }
