...
export async function adminGetPopups(
  page = 1,
  limit = 15,
): Promise<NormalizedPopupsResponse> {
  try {
    const { data } = await adminApiClient.get<PopupsResponse>(
      `/admin/popups?page=${page}&limit=${limit}`,
    );
    return {
    ...
    console.error(
      "[Popup Service] Failed to fetch popup campaigns, returning safe default NormalizedPopupsResponse:",
      error,
    );
    return {
      items: [],
...
export async function adminGetPopup(id: string): Promise<PopupCampaign | null> {
  try {
    ...
    console.error(
      `[Popup Service] Failed to fetch popup campaign ${id}:`,
      error,
    );
    return null;
  }
...
export async function adminCreatePopup(
  payload: Partial<PopupCampaign>,
): Promise<PopupCampaign | null> {
  try {
    const { data } = await adminApiClient.post<{ data: PopupCampaign }>(
      "/admin/popups",
      payload,
    );
    return data.data;
...
export async function adminUpdatePopup(
  ...
  try {
    const { data } = await adminApiClient.put<{ data: PopupCampaign }>(
      `/admin/popups/${id}`,
      payload,
    );
    return data.data;
    ...
    console.error(
      `[Popup Service] Failed to update popup campaign ${id}:`,
      error,
    );
    throw error;
  }
...
export async function adminDeletePopup(id: string): Promise<void> {
  try {
    ...
    console.error(
      `[Popup Service] Failed to delete popup campaign ${id}:`,
      error,
    );
    throw error;
  }
...
export async function adminTogglePopup(
  id: string,
  ...
  try {
    ...
    console.error(
      `[Popup Service] Failed to toggle active status for popup campaign ${id}:`,
      error,
    );
    throw error;
  }
