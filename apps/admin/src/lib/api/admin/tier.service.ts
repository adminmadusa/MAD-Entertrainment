...
export async function adminGetTiers(): Promise<AdminTier[]> {
  const { data } = await adminApiClient.get<{ data: AdminTier[] }>(
    "/admin/tiers",
  );
...
export async function adminCreateTier(
  payload: Partial<AdminTier>,
): Promise<AdminTier> {
  const { data } = await adminApiClient.post<{ data: { tier: AdminTier } }>(
    "/admin/tiers",
    payload,
  );
...
export async function adminUpdateTier(
  ...
  const { data } = await adminApiClient.patch<{ data: { tier: AdminTier } }>(
    `/admin/tiers/${id}`,
    payload,
  );
  return data.data.tier;
