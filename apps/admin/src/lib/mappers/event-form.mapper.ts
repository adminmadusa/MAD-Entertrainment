...
export function mapEventToFormValues(event: AdminEvent): EventFormValues {
  ...
  const overrides = (event.ticketOverrides || []).reduce<
    Record<string, { totalCapacity?: number; isActive?: boolean }>
  >((acc, item) => {
    acc[item.tier] = {
  ...
  return {
    ...
    coverImage:
      event.coverImage?.url && event.coverImage?.publicId
        ? {
    ...
    organizerName:
      (event as AdminEvent & { organizerName?: string }).organizerName || "",
    refundPolicy:
      (event as AdminEvent & { refundPolicy?: string }).refundPolicy || "",
    highlightsInput: (
      (event as AdminEvent & { highlights?: string[] }).highlights || []
    ).join(", "),
...
export function mapFormValuesToPayload(
  ...
  if (values.ticketingType === "profile") {
    ...
    payload.ticketOverrides = Object.entries(values.overrides)
      .map(([tier, vals]) => ({
        tier,
        totalCapacity:
          vals.totalCapacity !== undefined ? vals.totalCapacity : undefined,
        isActive: vals.isActive !== undefined ? vals.isActive : undefined,
      }))
      .filter(
        (item) =>
          item.totalCapacity !== undefined || item.isActive !== undefined,
      );
    payload.ticketTiers = [];
    return payload;
  }
  ...
  const ticketTiers = values.tiers.map((tier) => {
    const resolvedTierEnum = Object.values(TicketTier).includes(
  ...
  payload.totalCapacity = ticketTiers.reduce(
