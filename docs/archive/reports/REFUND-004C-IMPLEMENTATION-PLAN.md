...
|---|---|---|---|
| [refund.schema.ts](file:///Users/admin/Desktop/MAD%20Entertrainment/apps/server/src/models/refund.schema.ts) | Model Schema | Low | Add `gatewayRefundStatus`, `reconciledAt`, `webhookEventId` fields, and `gatewayRefundId` sparse index. |
| [refund.service.ts](file:///Users/admin/Desktop/MAD%20Entertrainment/apps/server/src/services/admin/refund.service.ts) | Admin Approval Service | High | 1. Write `gatewayRefundId` right after Phase 2 gateway success (minimizes crash window).<br/>2. Re-fetch booking status inside Phase 3 transaction (mitigates stale status crash).<br/>3. Prevent duplicate email triggers using `reconciledAt` flags. |
| [payment.service.ts](file:///Users/admin/Desktop/MAD%20Entertrainment/apps/server/src/services/public/payment.service.ts) | Webhook Service Logic | High | Implement Stripe and Razorpay webhook reconciliation handlers featuring payment serialization locks, amount-matched fallback lookups, booking cancellation execution, and gateway-initiated auto-creation logic. |
| [payment.controller.ts](file:///Users/admin/Desktop/MAD%20Entertrainment/apps/server/src/controllers/public/payment.controller.ts) | Webhook Controller Routing | Medium | Route Stripe and Razorpay refund events, parsing different payload structures correctly, and writing tracing metadata to `WebhookEvent`. |
| [payment.schema.ts](file:///Users/admin/Desktop/MAD%20Entertrainment/apps/server/src/models/payment.schema.ts) | Payment Schema | None | **No modifications required.** Enums and statuses already support refund states. Included for validation. |
| [webhook-event.schema.ts](file:///Users/admin/Desktop/MAD%20Entertrainment/apps/server/src/models/webhook-event.schema.ts) | WebhookEvent Schema | None | **No modifications required.** The schema already contains the `paymentId` and `rawPayload` (Mixed) fields needed for traceability. |
| [consistency.service.ts](file:///Users/admin/Desktop/MAD%20Entertrainment/apps/server/src/services/consistency.service.ts) | Watchdog Services | None | **No modifications required.** The watchdog (`repairStuckProcessingRefunds`) is compatible with the state machine as-is. |
