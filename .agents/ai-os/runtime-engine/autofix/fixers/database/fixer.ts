...
export class DatabaseFixer implements IFixer {
  id = 'DB-001';
  title = 'Database Mongoose Schema Fixer';
  category = 'database';
  ...
  async suggestFix(context: FixContext): Promise<AutoFixResult> {
    ...
    if (targetIdx >= 0 && targetIdx < lines.length) {
      ...
      const modifiedLine = originalLine.replace(/autoIndex:\s*true/g, 'autoIndex: false');
...
fixerRegistry.register(new DatabaseFixer());
